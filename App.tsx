...
import { LayoutDashboard, Ticket, FileSpreadsheet, RefreshCw, AlertCircle, Save, X, HardDrive, UserPlus, SlidersHorizontal, Database, RotateCcw } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const totalPoolSize = useMemo(() =>
    rewardTypes.reduce((sum, type) => sum + (rewardCounts[type] || 0), 0),
    [rewardCounts, rewardTypes]
  );
      ...
      <header className="px-6 py-4 flex items-center justify-between bg-white border-b-2 border-indigo-50 shrink-0 z-20">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold border transition-colors ${syncStatus === 'synced' ? 'bg-green-50 text-green-600 border-green-200' :
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl font-black text-sm transition-all shadow-md active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${view === 'dashboard'
          ...
          {view === 'gacha' && (
            <button onClick={() => confirm('確定要重置卡池嗎？所有卡片將會重新洗牌。') && initGacha()} className="p-2.5 text-orange-400 hover:text-orange-600 hover:bg-orange-50 rounded-2xl transition-all" title="重置卡池">
              <RotateCcw size={20} />
            </button>
          )}
          <button onClick={() => loadData()} className="p-2.5 text-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-2xl transition-all" title="重新同步資料">
